 ...
 * data), adjudicated by GenLayer, and released on Arc via RecurringMarket.recordDelivery.
 */
...
const LIVE_STORE = storePath("RM_LIVE_STORE", "rm-live.json");
let live = loadLive();
...
async function deliverOne(plan, index) {
  ...
  const inner = ethers.solidityPackedKeccak256(
    ...
    [CHAIN_ID, ADDR.recurringMarket, plan.planId, index, hash, score, verdict.adjudicationId],
  );
  const sig = await wallet.signMessage(ethers.getBytes(inner));
  await (await writer.recordDelivery(plan.planId, index, hash, score, verdict.adjudicationId, sig)).wait();
  deliveries[`${plan.planId}#${index}`] = {
    text, score, hash,
...
export function startRecurringMarket() {
  if (!wallet || !genlayerEnabled()) { console.log("[recurring] Arc relay signer or GenLayer adjudicator not configured — scheduler disabled."); return; }
