...
function createRecurring(networkId) {
  ...
  async function deliverOne(plan, index) {
    ...
    // Each scheduled drop is judged on its own by GenLayer validators, against the
    // plan's rubric, before this slice of escrow is released.
    ...
    const verdict = prepared.ungradeable
      ...
      : await adjudicateTask(ctx, {
          ...
          sourceId: ethers.id(`recurring:${plan.planId}:${index}`),
    ...
    const inner = recurringDeliveryDigest(ctx, { planId: plan.planId, index, deliverableHash: hash, score, genLayerDecisionId: verdict.adjudicationId });
    const sig = await wallet.signMessage(ethers.getBytes(inner));
    const writer = ctx.contract("recurringMarket", "recurringMarket", wallet);
    await (ctx.genlayerDecisionBinding
      ? await writer.recordDelivery(plan.planId, index, hash, score, verdict.adjudicationId, sig)
      : await writer.recordDelivery(plan.planId, index, hash, score, sig)).wait();
    deliveries[dkey(plan.planId, index)] = {
...
export function startRecurringMarket(networkId = DEFAULT_NETWORK) {
  ...
  if (!genlayerEnabled()) {
    console.log(`[recurring:${networkId}] GenLayer adjudicator not configured — scheduler disabled.`);
    svc.refresh();
    return;
  }
