...
const RD_DISPUTE_STORE = process.env.RD_DISPUTE_STORE || "./recurring-disputes.json";
...
app.post("/api/recurring-dispute", async (req, res) => {
  try {
    ...
    store[`${planId}#${index}`] = { planId, index: Number(index), complaint: (complaint || "").slice(0, 500), upheld, juryNote, reporter: reporter || null, atMs: Date.now() };
    try {
...
const FLAGS_STORE = process.env.FLAGS_STORE || "./flags.json";
function loadFlags() {
  try {
...
// Operator-only: the full flag queue for review (guarded by ADMIN_SECRET).
