...
/**
 * Build the Circle execution payload. Prefer pre-encoded `callData` (viem-encoded
 * on the client) — Circle's `abiParameters` only supports string/int/bool/array,
 * NOT tuples/structs, so a call like createPlan(...,(string,string,...)) silently
 * fails via abiFunctionSignature+abiParameters. callData sidesteps that entirely.
 */
function execPayload({ abiFunctionSignature, abiParameters, callData }) {
  return callData ? { callData } : { abiFunctionSignature, abiParameters: abiParameters ?? [] };
}

/** Contract-execution challenge addressed by userToken (email-login users). */
export async function contractExecutionChallengeByToken(userToken, walletId, contractAddress, opts) {
  const c = uc();
 ...
 * browser signs with the user's PIN. Accepts either raw `callData` (preferred)
 * or an `abiFunctionSignature` + `abiParameters` pair.
 */
export async function contractExecutionChallenge(userId, walletId, contractAddress, opts) {
  const c = uc();
  const res = await c.createUserTransactionContractExecutionChallenge({
    userId,
    walletId,
