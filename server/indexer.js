...
const EVENTS = {
  ...
  verifierBridge: [
    ...
    // Post-GenLayer shape. Adding a parameter changes topic0, so the two are
    // different events to an ABI decoder and BOTH must be listed: the wildcard scan
    // parses by topic0, and dropping the old one would erase every settlement that
    // happened before this network was redeployed. Same reason as the legacy V4
    // event above.
  ...
  async function buildIndex() {
    ...
    for (const log of verifierLogs) {
      ...
      t.attestation = {
        ...
        // Present only for settlements made against a redeployed bridge.
        genLayerDecisionId: a.genLayerDecisionId ?? null,
      };
    ...
    for (const log of disputeLogs) {
      ...
      if (log.name === "DisputeOpened") {
        ...
        if (dz) {
          ...
          dz.genLayerDecisionId = a.genLayerDecisionId ?? null;
        }
      }
    }
