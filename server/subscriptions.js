...
import { provider, ADDR, ABI, USDC_DECIMALS, CHAIN_ID } from "./chain.js";
 ...
 * finalize a GenLayer verdict, relay it, and call recordDelivery —
...
const LIVE_STORE = storePath("SUB_LIVE_STORE", "sub-live.json");
let live = loadLive();
...
function deliveryList(subId, count) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (d) list.push({
      index: i, score: d.score, atMs: d.atMs, hash: d.hash,
      genLayerDecisionId: d.genLayerDecisionId,
...
async function deliverOne(sub, index) {
  ...
  const verdict = await adjudicateTask({
    ...
    sourceContract: ADDR.subscriptionManager,
  ...
  const inner = ethers.solidityPackedKeccak256(
    ...
    [CHAIN_ID, ADDR.subscriptionManager, sub.subId, index, hash, score, verdict.adjudicationId],
  );
  const sig = await wallet.signMessage(ethers.getBytes(inner));
  const tx = await writer.recordDelivery(sub.subId, index, hash, score, verdict.adjudicationId, sig);
  await tx.wait();
  deliveries[`${sub.subId}#${index}`] = {
    text, score, hash,
...
export function startScheduler() {
  if (!wallet || !genlayerEnabled()) {
    console.log("[subscriptions] Arc relay signer or GenLayer adjudicator not configured — scheduler disabled (read API still served).");
    return;
  }
