...
import { prepareForAdjudication, produceWork } from "./score.js";
import { adjudicateTask, genlayerEnabled } from "./genlayer.js";
...
function createSubscriptions(networkId) {
  ...
  async function deliverOne(sub, index) {
    ...
    // Each scheduled delivery is judged on its own by GenLayer validators, against
    // the plan's rubric, before this slice of prepaid escrow is released.
    ...
    const verdict = prepared.ungradeable
      ...
      : await adjudicateTask(ctx, {
          // bytes32 on the mirror, so it must be a hash. The case id already folds
          // in the chain id, so this does not need the network scoping dkey() adds.
          sourceId: ethers.id(`subscription:${sub.subId}:${index}`),
          sourceContract: ctx.ADDR.subscriptionManager,
          requester: sub.subscriber,
          agent: sub.agent,
          title: sub.title,
    ...
    const inner = subscriptionDeliveryDigest(ctx, { subId: sub.subId, index, deliverableHash: hash, score, genLayerDecisionId: verdict.adjudicationId });
    ...
    const tx = ctx.genlayerDecisionBinding
      ? await writer.recordDelivery(sub.subId, index, hash, score, verdict.adjudicationId, sig)
      : await writer.recordDelivery(sub.subId, index, hash, score, sig);
    await tx.wait();
    deliveries[dkey(sub.subId, index)] = {
      text, score, hash, network: networkId, atMs: Date.now(),
      genLayerDecisionId: verdict.adjudicationId, genlayerTxHash: verdict.genlayerTxHash,
    };
    saveStore();
...
export function startScheduler(networkId = DEFAULT_NETWORK) {
  ...
  // No GenLayer operator means no verdict, so every scheduled delivery would be
  // produced, fail to adjudicate, and be produced again on the next tick.
  if (!genlayerEnabled()) {
    console.log(`[subscriptions:${networkId}] GenLayer adjudicator not configured — scheduler disabled (read API still served).`);
    svc.refresh(); // still warm the read cache
    return;
  }
