...
export const ABI = {
  ...
  /**
   * GenLayer-era variants of the four settlement ABIs. The redeployed contracts take
   * the finalized decision id as an extra argument, bind it into the signed digest and
   * emit it — so the call shape, the return shape of `getDispute`, and three event
   * signatures all differ from what is deployed today. `ctx.contract()` picks between
   * these and the pairs above on `genlayerDecisionBinding`; see server/digests.js for
   * why that is a property of the deployment rather than of the repo.
   *
   * Each variant keeps the PRE-GenLayer event signature alongside the new one, so a
   * cold index on a redeployed network still decodes everything that settled before
   * the redeploy instead of dropping it on the floor.
   */
  verifierBridgeGenlayer: [
    "function submitVerification(bytes32,address,address,bool,uint8,bytes32,bytes32,bytes)",
    "function processed(bytes32) view returns (bool)",
  ],
  subscriptionManagerGenlayer: [
  ...
  disputeManagerGenlayer: [
    "function resolveDispute(bytes32 disputeId, bool upheld, string juryNote, bytes32 genLayerDecisionId, bytes signature)",
  ...
  recurringMarketGenlayer: [
    "function bid(bytes32 planId, uint256 price, uint256 etaSeconds)",
    "function award(bytes32 planId)",
    "function recordDelivery(bytes32 planId, uint32 index, bytes32 deliverableHash, uint8 score, bytes32 genLayerDecisionId, bytes signature)",
    "function getPlan(bytes32) view returns (address requester, address agent, uint256 perDelivery, uint256 price, uint32 total, uint32 done, uint256 escrowed, uint256 minReputation, uint8 status)",
    "function bidCount(bytes32) view returns (uint256)",
    "function bidsOf(bytes32, uint256) view returns (address agent, uint256 price, uint256 score)",
    "event PlanCreated(bytes32 indexed planId, address indexed requester, uint256 perDelivery, uint32 total, uint256 minReputation, string title, string brief, string rubric, string taskType, string schedule)",
...
export function getChainCtx(networkId = DEFAULT_NETWORK) {
   ...
   *
   * Where a `<abiKey>Genlayer` variant exists it is used on networks whose contracts
   * have been redeployed with the decision-id binding, and the original everywhere
   * else. Callers name the logical contract and never the tier, so a network flips
   * shape by flipping one flag in networks.js.
   */
  function contract(key, abiKey, signer) {
    requireAddresses([key]);
    const genlayerAbiKey = `${abiKey}Genlayer`;
    const abi = net.genlayerDecisionBinding && ABI[genlayerAbiKey] ? ABI[genlayerAbiKey] : ABI[abiKey];
    return new ethers.Contract(ADDR[key], abi, signer ?? provider);
  }
  ...
  const ctx = {
    ...
    /** True where the DEPLOYED contracts also bind the GenLayer decision id into
     *  the digest, and take it as a call argument. Consumed by server/digests.js. */
    genlayerDecisionBinding: Boolean(net.genlayerDecisionBinding),
    queryLogsChunked,
    scanRange,
    readTaskMeta,
    readAssignedAgent,
