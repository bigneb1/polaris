...
// Same fixed start block indexer.js uses for the full-history index (deploy
// block) — a single-task lookup (readTaskMeta) needs this, not a rolling
// window, since a task doesn't stop needing verification just because it's
// older than INDEX_LOOKBACK_BLOCKS. See docs/AUDIT_REPORT.md, "readTaskMeta
// lookback bug" (found live, post-audit).
const FROM_BLOCK = process.env.INDEX_FROM_BLOCK ? Number(process.env.INDEX_FROM_BLOCK) : null;
 ...
 * @param {number} [fromBlock] absolute start block — takes precedence over `lookback` when set
 */
...
export async function queryLogsChunked(contract, filter, lookback, fromBlock) {
...
export async function readTaskMeta(taskId) {
  ...
  // Explicit fromBlock (deploy block, if set) rather than the default rolling
  // lookback window: a task's TaskSubmitted event doesn't become unfindable
  // just because the chain has since advanced past INDEX_LOOKBACK_BLOCKS —
  // Arc's block rate means that window covers only ~1-2 days, so without this
  // any task older than that would 404 out of /api/verify forever, stuck at
  // ASSIGNED (confirmed live: this is exactly what was happening).
