...
/**
 * Open a staked dispute on a settled task. The requester stakes a stablecoin bond and
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-background/70 p-4 backdrop-blur-sm" onClick={onClose}>
      <div className="rounded-[4px] border border-border bg-card flex max-h-[90vh] w-full max-w-lg flex-col overflow-hidden p-0" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between border-b border-border px-5 py-4">
          <div className="inline-flex items-center gap-2 text-sm font-semibold text-foreground"><Scale size={15} className="text-secondary" /> Dispute deliverable</div>
        ...
        <div className="min-w-0 overflow-y-auto px-5 py-4">
          {phase === "done" && verdict ? (
            <div className="flex flex-col gap-3">
              <div className={`inline-flex items-center gap-2 rounded-xl border px-3 py-2.5 text-sm font-semibold ${verdict.upheld ? "border-success/40 bg-success/5 text-success" : "border-destructive/40 bg-destructive/5 text-destructive"}`}>
            ...
            <div className="flex flex-col gap-3">
              ...
              <label className="block">
                <div className="field-label mb-1.5">Dispute bond ({symbol})</div>
                <input type="number" min="5" className="field" value={bond} onChange={(e) => setBond(e.target.value)} />
              </label>
              <div className="rounded-xl border border-border bg-muted p-3 text-xs">
                <div className="flex items-center justify-between">
                  ...
                  <USDCAmount amount={parseFloat(bond) || 0} size="sm" className="text-foreground" />
                </div>
              </div>
        ...
        <div className="flex items-center justify-end gap-2 border-t border-border px-5 py-4">
          {phase === "done" || phase === "pending" ? (
            <button onClick={onClose} className="tool-btn-primary">Close</button>
          ) : (
            <>
              <button onClick={onClose} className="tool-btn" disabled={phase !== "form"}>Cancel</button>
              <button onClick={submit} disabled={phase !== "form" || !reason.trim() || !(parseFloat(bond) > 0)} className="tool-btn-primary">
