 ...
 *
 * We send viem-encoded `callData` rather than Circle's abiFunctionSignature +
 * abiParameters. Circle's abiParameters only supports string/int/bool/array —
 * NOT tuples/structs — so a call carrying a struct (e.g. createPlan /
 * createSubscription with PlanMeta) silently fails after the approve. callData
 * carries the exact ABI encoding, matching the passkey path (circleWallet.ts).
 */
export async function ucWrite(session: UcSession, call: Call): Promise<void> {
  const callData = encodeFunctionData({ abi: call.abi, functionName: call.functionName, args: call.args });
