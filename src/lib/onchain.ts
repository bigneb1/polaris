...
async function fetchPlans(): Promise<RecurringPlan[]> {
  const r = await fetch(`${API_URL}/api/recurring-plans`);
...
export function useRecurringPlans(filter?: { requester?: string }) {
  const q = useQuery({ queryKey: ["polaris-recurring-plans"], refetchInterval: REFETCH_MS, queryFn: fetchPlans });
  let plans = q.data ?? [];
  if (filter?.requester) plans = plans.filter((p) => p.requester.toLowerCase() === filter.requester!.toLowerCase());
  return { plans, isLoading: q.isLoading };
}
