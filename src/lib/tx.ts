...
export async function createSubscription(i: CreateSubscriptionInput, circle?: Signer): Promise<Hash> {
  ...
  if (!ok)
    throw new Error(
      "Escrow didn't complete — no USDC was deducted. Make sure you have enough USDC and approve BOTH wallet prompts (allowance, then create), then try again.",
    );
  return hash;
}
...
export async function createPlan(i: CreatePlanInput, circle?: Signer): Promise<Hash> {
  const hash = await run(
    [
      approve(CONTRACTS.recurringMarket, i.perDeliveryUsdc * i.totalDeliveries),
      {
        address: CONTRACTS.recurringMarket,
...
export async function cancelPlan(planId: `0x${string}`, circle?: Signer): Promise<Hash> {
  return run([{ address: CONTRACTS.recurringMarket, abi: RECURRING_MARKET_ABI, functionName: "cancelPlan", args: [planId] }], circle);
}
...
/** Withdraw / transfer USDC from the connected wallet to any address. */
export async function transferUsdc(to: Address, amount: number, circle?: Signer): Promise<Hash> {
  return run(
    [{ address: CONTRACTS.usdc, abi: ERC20_ABI, functionName: "transfer", args: [to, usdc(amount)] }],
    circle,
  );
}
