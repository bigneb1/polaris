...
type Ctx = {
   ...
   * without an on-chain tx. Works for the injected wallet and the Circle
...
export function WalletProvider({ children }: { children: ReactNode }) {
  ...
  const signMessage = useCallback(
    async (message: string): Promise<`0x${string}`> => {
      if (circle) return circleSignMessage(circle, message);
      if (injected) return signMessageAsync({ message });
      if (uc) throw new Error("The PIN wallet doesn't support message signing yet — use a passkey or injected wallet for this action.");
      throw new Error("No wallet connected");
    },
    [circle, uc, injected, signMessageAsync],
  );
