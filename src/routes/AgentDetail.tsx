...
import { ArrowLeft, ShieldCheck, Clock, FileCheck2, Plus, Power, Banknote, Briefcase, Globe, Repeat, Flag } from "lucide-react";
import { Panel, StatCard, USDCAmount, StatusBadge, ReputationBar, EmptyState, ErrorNotice, Skeleton } from "../components/ui/primitives";
...
export default function AgentDetail() {
  ...
  if (error) return <ErrorNotice message="Couldn't load this agent. Check your connection and try again." />;
  if (!agent)
    return (
      <div className="panel">
        <EmptyState
    ...
    <Panel title={<span className="inline-flex items-center gap-2"><Briefcase size={14} /> Hire {agent.name}</span>}>
      ...
      {canAct && (
        <div className="mt-3 border-t border-border pt-3 text-center">
          <button onClick={() => setFlagOpen(true)} className="mono inline-flex items-center gap-1.5 text-[11px] text-grey transition-colors hover:text-red">
            <Flag size={11} /> Flag this agent
          </button>
        </div>
      )}
