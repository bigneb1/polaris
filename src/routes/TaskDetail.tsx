...
export default function TaskDetail() {
  ...
  if (error) return <ErrorNotice message="Couldn't load this task. Check your connection and try again." />;
  if (!task)
    return (
      <div className="panel">
        <EmptyState
    ...
    <div>
      ...
      <div className="grid gap-6 lg:grid-cols-[1.5fr_1fr]">
        ...
        <div className="flex min-w-0 flex-col gap-6">
          ...
          {task.attestation && (
            <Panel title={<span className="inline-flex items-center gap-2"><ShieldCheck size={14} /> Onchain settlement attestation</span>}>
              <div className="flex flex-col gap-4">
                ...
                {task.attestation.genLayerDecisionId && (
                  <div>
                    <div className="eyebrow mb-1">Finalized GenLayer decision</div>
                    <div className="mono break-all text-[11px] text-grey-l">{task.attestation.genLayerDecisionId}</div>
                  </div>
                )}
                <a
                ...
                <p className="mono text-[11px] leading-relaxed text-grey">
                  Adjudicated by GenLayer validators against the rubric, finalized through consensus, and relayed
                  onchain via VerifierBridge. Status: <span className="text-green">SETTLED</span>.
                </p>
              </div>
            </Panel>
          )}
          ...
          {/* Dispute (Phase C) — show latest status, and let the requester dispute
              up to MAX_DISPUTES (3) times per task. */}
          {task.dispute && (
            <Panel title={<span className="inline-flex items-center gap-2"><Scale size={14} /> Dispute</span>}>
              <div className="flex flex-col gap-3">
                <Link to={`/dispute/${task.dispute.disputeId}`} className={`inline-flex w-fit items-center gap-2 rounded-md border px-2.5 py-1 text-xs font-semibold transition-opacity hover:opacity-80 ${
          ...
          {isRequester && task.status === "SETTLED" && task.assignedAgent && (() => {
            ...
            const used = task.disputesByRequester?.[me] ?? 0;
            // Block a new dispute while THIS user's dispute is still under review.
            const pending = (task.disputes ?? (task.dispute ? [task.dispute] : [])).some(
              (d) => d.status === "OPEN" && (d.requester?.toLowerCase() ?? me) === me,
            );
            const remaining = MAX_DISPUTES - used;
            if (remaining <= 0)
              return (
                <div className="panel p-4">
                  <div className="mono text-[11px] text-grey">You've used all {MAX_DISPUTES} disputes for this task.</div>
                </div>
              );
            return (
              <div className="panel flex flex-wrap items-center justify-between gap-3 p-4">
                <div className="min-w-0">
                  ...
                  <div className="mono text-[11px] text-grey">
                    Stake a bond and let the AI jury re-judge it. {used > 0 && `· ${remaining} of ${MAX_DISPUTES} left`}
                  </div>
                </div>
                <button onClick={() => setDisputeOpen(true)} disabled={pending} className="btn-ghost btn-sm">
                  <Scale size={13} /> {pending ? "Review pending…" : "Dispute"}
                </button>
              </div>
            );
          })()}
...
function DeliverablePanel({ taskId, agent, status, title }: { taskId: `0x${string}`; agent?: `0x${string}`; status: import("../lib/types").TaskStatus; title?: string }) {
  ...
  const finished = status === "SETTLED" || status === "COMPLETED" || status === "CANCELLED";
    ...
    >
      {loading ? (
        ...
        <EmptyState
          ...
          message="Viewing a deliverable requires signing a short message with the requester's wallet. If you're on the PIN wallet, switch to a passkey or injected wallet to view it, then try again."
        />
      ) : text ? (
        <div className="flex flex-col gap-3">
          {agent && (
