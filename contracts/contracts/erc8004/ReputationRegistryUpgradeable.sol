...
// Vendored verbatim from the ERC-8004 reference implementation
// (github.com/erc-8004/erc-8004-contracts, contracts/ReputationRegistryUpgradeable.sol).
//
// WHY VENDORED: Polaris needs functional ERC-8004 registries on BOT Chain, and the
// canonical deployment isn't usable there — on BOT mainnet the vanity addresses are
// ERC-1967 proxies still delegating to a `MinimalUUPSMainnet` placeholder (every
// call reverts, verified on-chain), and BOT testnet has no deployment at all. The
// upgrade that would activate them is signed by the standard's owner key, which
// Polaris does not hold. So Polaris deploys these same reference implementations at
// its own addresses: interface-compliant, just not at the canonical vanity address.
// Registry addresses are configuration (see src/lib/networks/), so if the canonical
// proxies are activated later, pointing at them is a config change.
//
// DO NOT EDIT — keep byte-identical to upstream so the deployment stays standard.
