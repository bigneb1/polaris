...
interface INativeEscrow {
    function lockFunds(bytes32 taskId, address requester, uint256 amount) external payable;
...
/**
 * NativeTaskRegistry — task lifecycle for a market denominated in the chain's
 * NATIVE coin. The native-value twin of TaskRegistry, used on BOT Chain. Arc keeps
 * TaskRegistry unchanged.
 *
 * Identical lifecycle, events and authorization to TaskRegistry — so BidEngine,
 * VerifierBridge and the event indexer work against either registry unchanged. The
 * only difference: `submitTask` / `submitDirectTask` are payable and forward the
 * value they receive to the escrow, requiring `msg.value == budget`. With a native
 * coin there is no allowance to pull from, so the budget must arrive with the call —
 * which also removes the two-step approve+post flow (and with it the "approve mined
 * but create didn't" failure mode) on this network.
 */
contract NativeTaskRegistry {
    INativeEscrow public escrow;
    ...
    struct Task {
        bytes32 taskId;
        address requester;
        uint256 budgetUsdc; // name kept for ABI/indexer compatibility; native wei here
        uint256 deadline;
    ...
    /// Post a task, sending the budget with the call.
    function submitTask(
        bytes32 taskId,
        uint256 budgetUsdc,
        uint256 deadline,
    ...
    function _lock(bytes32 taskId, uint256 budgetUsdc, uint256 deadline, uint256 minReputation) internal {
        ...
        require(msg.value == budgetUsdc, "Value must equal budget");
        // Forward the value on; the escrow credits msg.sender as the refund payee.
        escrow.lockFunds{value: msg.value}(taskId, msg.sender, budgetUsdc);
        tasks[taskId] = Task({
            taskId: taskId,
